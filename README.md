...
> 아래 이미지는 `portfolio-images/` 폴더에 `01-landing.png` ~ `05-swagger.png` 이름으로 저장한 뒤 사용하시면 됩니다.  
> 개인정보(닉네임, 이메일 등)가 보이면 모자이크 처리 후 사용하세요.

### 1. 랜딩 / 메인

![랜딩 / 메인 화면](portfolio-images/01-landing.png)

- **캡처 대상**: 첫 진입 시 랜딩 또는 메인 게임 진입 화면
- **포인트**: Glassmorphism UI, CTA 버튼

---

### 2. 게임 플레이 화면

![게임 플레이](portfolio-images/02-game.png)

- **캡처 대상**: 60초 게임 진행 중 — 차트 + 롱/숏·포지션 종료 버튼
- **포인트**: 실시간 차트, 잔고, 포지션 상태

---

### 3. 랭킹

![랭킹](portfolio-images/03-ranking.png)

- **캡처 대상**: 랭킹 패널 또는 랭킹 페이지
- **포인트**: 순위, 닉네임(필요 시 마스킹), 점수/수익률

---

### 4. 마이페이지

![마이페이지](portfolio-images/04-mypage.png)

- **캡처 대상**: 마이페이지 — 통계, 업적, 매매 내역 요약 등
- **포인트**: 대시보드형 구성, 업적/칭호, 그래프

---

### 5. API 문서 (Swagger)

![Swagger API 문서](portfolio-images/05-swagger.png)

- **캡처 대상**: Swagger UI (`/api-docs`) 화면
- **포인트**: API 문서화·인증(Bearer) 표시
...
| **API 문서** | Swagger UI로 인증 포함 API 문서 제공 |
...
- 체험용 URL을 공개할 경우 여기에 한 줄로 기재 (예: `https://charttrainer.example.com`).
- 관리자/내부용 URL은 기재하지 마세요.

---

## 📝 면접 시 참고

- **소스코드**: 보안 및 운영 정책상 비공개 저장소로 관리 중입니다.
- **코드 리뷰**: 면접 시 화면 공유로 저장소 구조·핵심 모듈·아키텍처를 보여드릴 수 있습니다.
